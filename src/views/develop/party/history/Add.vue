...
<script lang="ts" setup>
  ...
  import { addOrUpdateHistoryApi, historyFindOneApi } from '/@/api/party/history';
  import { HistoryItem } from '/@/api/party/model/historyModel';
  ...
  const sourceData = ref({} as HistoryItem);
  if (id) {
    isEdit.value = true;
  }
  ...
  onMounted(async () => {
    if (isEdit.value) {
      try {
        const res = await historyFindOneApi(id);
        fileList.value = [
          {
        ...
        console.error(error);
        router.replace('/study/partyHistory/list');
      }
    }
  });
  ...
  const submit = async (fromData: HistoryItem) => {
    ...
    if (images[0]) {
      ...
      console.log({
        ...(sourceData.value as HistoryItem),
        ...fromData,
      });
      const res = await addOrUpdateHistoryApi({
        ...(sourceData.value as HistoryItem),
        ...fromData,
        poster: images[0],
      });
