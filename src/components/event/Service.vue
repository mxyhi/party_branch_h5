...
async function load() {
  const { total, data } = await getServiceList({ page, limit: 10, id: userId });
...
const joinEvent = (e, item) => {
  ...
  if (item.isParticipateIn) {
    fetchCancelService(item.registrationInformation.id).then(res => {
      console.log(res);
      item.isParticipateIn = false;
      item.registrationInformation = res;
    });
  } else {
    fetchJoinService({ id: item.id, userId }).then(res => {
      console.log(res);
