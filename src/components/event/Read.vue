<script setup>
import { ref } from 'vue';
import { getReadList, fetchCancelRead, fetchJoinRead } from '@/api/event/read';
...
async function load() {
  const { total, data } = await getReadList({ page, limit: 10, id: userId });
...
const joinEvent = (e, item) => {
  ...
  if (item.isParticipateIn) {
    fetchCancelRead(item.registrationInformation.id).then(res => {
      console.log(res);
      item.isParticipateIn = false;
      item.registrationInformation = res;
    });
  } else {
    fetchJoinRead({ id: item.id, userId }).then(res => {
      console.log(res);
